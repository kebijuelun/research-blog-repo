...
![Figure 3](fig/ERNIE-4.5-21B-A3B-PT-layer_1.png)
> 图解：ERNIE 早层，剪枝保留流形几何结构，合并将分布压向中心。

![Figure 4](fig/Llama-4-Scout-17B-16E-Instruct-layer_47.png)
...
![Figure 5](fig/combined-all-tasks_qwen_and_glm.png)
...
![Figure 6](fig/ngram_diversity.png)
> 图解：N-Gram 多样性。合并模型输出更重复，REAP 更接近原模型。

![Figure 7](fig/cross_perplexity.png)
> 图解：交叉困惑度。合并模型的输出偏离 baseline 更严重。

![Figure 8](fig/jsd_small.png)
> 图解：Logit 分布的 JSD 随输出步数增长，合并模型更快偏离原模型。

![Figure 9](fig/combined_weight_dist.png)
...
![Figure 10](fig/restricted_cluster_size_m_smoe.png)
...
![Figure 11](fig/dataset-ablation-all-models.png)
> 图解：用通用数据（c4）校准时，代码生成质量严重崩溃，很多模型甚至输出不可用。

![Figure 12](fig/bar-combined-dataset-all-tasks-Qwen3-30B-A3B.png)
