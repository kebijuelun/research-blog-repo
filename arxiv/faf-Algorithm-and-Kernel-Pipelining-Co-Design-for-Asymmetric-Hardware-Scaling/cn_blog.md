...
![Figure: FA4 Forward Pipeline](Figures/FA4_FWD_p3.png)
...
![Forward non-causal](Figures/fa4_fwd_causalFalse_hdim128_updated.png)

> 图解：横轴是 sequence length，纵轴是 TFLOPS。非因果场景下，FA4 在中长序列上整体领先，体现了 pipeline 与非 matmul 优化在高负载下的收益。

![Forward causal](Figures/fa4_fwd_causalTrue_hdim128_updated.png)

> 图解：同样是 TFLOPS 对比，因果掩码下 FA4 的优势更明显，与论文中的 LPT 调度策略（优先处理长任务）一致。

![Forward (192,128) head](Figures/fa4_fwd_causalTrue_hdim192128_updated.png)

> 图解：这是更贴近 DeepSeek V3 风格的 $(192,128)$ 头维配置。FA4 在这类“非标准头维”上仍保持较高效率，说明框架泛化性较好。

![Backward non-causal](Figures/fa4_bwd_causalFalse_hdim128_updated.png)

> 图解：反向非因果场景中，FA4 在长序列下持续领先，符合 2-CTA 降低 SMEM 与 atomic 压力的设计预期。

![Backward causal](Figures/fa4_bwd_causalTrue_hdim128_updated.png)

> 图解：反向因果场景同样受益，说明调度与流水重排并非仅在单一工作负载下有效。

![Deterministic backward ablation](Figures/causal_bwd_det_FA4.png)
...
![Deterministic backward non-causal](Figures/non-causal_bwd_det_FA4.png)
